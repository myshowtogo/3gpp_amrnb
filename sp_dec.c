                    ...
                    Word32 A_t[])
{
   ...
   st->log_pg_mean = (Word16)(((29491 * st->log_pg_mean) >> 15) + ((3277 * log_pg) >> 15));
                        ...
                        enum RXFrameType frame_type, Word32 synth[], Word32 A_t[])
{
   ...
   for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      gain_code_mix = Cb_gain_average(st->Cb_gain_averState, mode, gain_code,
                                      lsf_i, st->lsp_avg_st->lsp_meanSave, bfi, st->prev_bf, pdfi, st->prev_pdf, st->inBackgroundNoise, st->voicedHangover);
      ...
      if (((mode == MR475) || (mode == MR515) || (mode == MR59)) & (st->voicedHangover > 5) & (st->inBackgroundNoise != 0) & (st->state < 4) & (((pdfi != 0) & (st->prev_pdf != 0)) || bfi != 0 || st->prev_bf != 0))
      {
         carefulFlag = 0;
    ...
    */
   st->inBackgroundNoise = Bgn_scd(st->background_state, &(st->ltpGainHistory[0]), &(synth[0]), &(st->voicedHangover));
 ...
 *    void
 */
void Speech_Decode_Frame(void *st, enum Mode mode, Word16 *parm, enum RXFrameType frame_type, Word16 *synth)
{
...
static int Decoder_amr_init(Decoder_amrState **state)
{
   ...
   *state = NULL;
   ...
   /* gcPredInit */
   /* allocate memory */
   if ((s->pred_state = (gc_predState *)malloc(sizeof(gc_predState))) == NULL)
   {
   ...
   /* lsp_avgInit */
   /* allocate memory */
   if ((s->lsp_avg_st = (lsp_avgState *)malloc(sizeof(lsp_avgState))) == NULL)
   {
   ...
   /* Bgn_scdInit */
   /* allocate memory */
   if ((s->background_state = (Bgn_scdState *)malloc(sizeof(Bgn_scdState))) == NULL)
   {
   ...
   /* phDispInit */
   /* allocate memory */
   if ((s->ph_disp_st = (ph_dispState *)malloc(sizeof(ph_dispState))) == NULL)
   {
   ...
   /* dtxDecInit */
   /* allocate memory */
   if ((s->dtxDecoderState = (dtx_decState *)malloc(sizeof(dtx_decState))) == NULL)
   {
...
void *Speech_Decode_Frame_init(void)
{
   ...
   if (Decoder_amr_init(&s->decoder_amrState) || Post_Filter_init(&s->post_state) || Post_Process_init(&s->postHP_state))
   {
